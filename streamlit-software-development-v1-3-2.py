...
if app_version:
    # Project Requirements Section (new)
    st.header("Project Requirements (requirements.txt)")
    if app_version in st.session_state.requirements_dict and st.session_state.requirements_dict[app_version].strip():
        with st.expander("Current saved requirements.txt", expanded=False):
            st.code(st.session_state.requirements_dict[app_version], language="text")

    requirements_text = st.text_area(
        "Enter or update requirements.txt content:",
        height=150,
        placeholder="numpy==1.24.0\npandas>=2.0.0\nstreamlit==1.28.0\nreportlab==4.0.0\nstreamlit-ace>=0.1.5\n...",
        help="Paste or enter the dependencies from your requirements.txt file. Saving will overwrite any existing requirements for this app version."
    )
    if st.button("Save Requirements"):
        if app_version:
            st.session_state.requirements_dict[app_version] = requirements_text
