...
if app_version:
    ...
    st.subheader("Upload Supporting Images")
    uploaded_images = st.file_uploader(
        "Upload images for regression notes",
        type=['png', 'jpg', 'jpeg'],
        accept_multiple_files=True,
        help="Upload screenshots or relevant images"
    )
    ...
    if st.button("Save Regression Testing Notes"):
        ...
        if uploaded_images:
            if app_version not in st.session_state.image_dict:
                st.session_state.image_dict[app_version] = []
            for img in uploaded_images:
                st.session_state.image_dict[app_version].append({
                    'data': img.getvalue(),
                    'name': img.name,
                    'type': img.type
                })
...
for app_version in st.session_state.task_list:
    ...
    # Display text inputs and images
    if app_version in st.session_state.text_dict:
        st.write("#### Notes:")
        for text in st.session_state.text_dict[app_version]:
            st.write(f"- {text}")
        if app_version in st.session_state.image_dict:
            st.write("Supporting Images:")
            for img_data in st.session_state.image_dict[app_version]:
...
if st.button("Generate PDF"):
    ...
    for app_version in st.session_state.task_list:
        ...
        if app_version in st.session_state.text_dict:
            ...
            if app_version in st.session_state.image_dict:
                pdf_elements.append(Paragraph("Supporting Images:", styles['Heading3']))
                for img_data in st.session_state.image_dict[app_version]:
                    img_buffer = BytesIO(img_data['data'])
                    pdf_img = Image(img_buffer, width=200, height=150)
                    pdf_elements.append(Paragraph(f"Image: {img_data['name']}", styles['Normal']))
