...
def create_reportlab_gui(index):
    st.subheader(f"Iteration #{index + 1}")
...
def generate_pdf():
    ...
    for i, content in enumerate(st.session_state.reportlab_instances):
        ...
        c.drawString(100, y_position, f"Iteration #{i + 1}:")
        y_position -= 20
...
# Generate codebase for display
def generate_codebase():
    code = ""
    for i, content in enumerate(st.session_state.reportlab_instances):
        first_line = content.split('\n')[0].strip()
        app_name_version = first_line[1:].strip()
...
# Function to display AI generated code with delete button
...
# Renumber AI Codebase list when an item is deleted
...
if st.button("Generate Code with AI"):
    if st.session_state.reportlab_instances and google_api_key:
        try:
            ...
            if ai_generated_code:
                current_iteration = {
                    ...
                    "IDE/Text Editor": ide,
                    "Framework": framework,
                    "Iterations": st.session_state.reportlab_instances.copy()
                }
        ...
        st.warning("Please add at least one iteration and enter API Key first.")

# Clear Current Iterations Button
if st.button("Clear Current Iterations"):
    st.session_state.reportlab_instances = []
    st.rerun()
