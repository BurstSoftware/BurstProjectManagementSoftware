...
if st.button("Save Modules/Frameworks"):
    ...
    if app_version:
        st.session_state.modules_dict[app_version] = modules_input
...
if app_version:
    ...
    # Multiple code editors for different languages
    ...
    for i in range(st.session_state.code_sections):
        ...
        if st.button(f"Save {code_type} Code Section {i+1}"):
            if app_version not in st.session_state.code_dict:
                st.session_state.code_dict[app_version] = []
            st.session_state.code_dict[app_version].append((code_type, code))
...
if st.button("Generate PDF"):
    ...
    for app_version in st.session_state.task_list:
        ...
        # Add modules/frameworks
        if app_version in st.session_state.modules_dict:
            pdf_elements.append(Paragraph(
                f"Modules/Frameworks: {st.session_state.modules_dict[app_version]}",
                styles['Normal']
            ))
            pdf_elements.append(Spacer(1, 10))
        ...
        if app_version in st.session_state.code_dict:
            ...
            for i, (code_type, code) in enumerate(st.session_state.code_dict[app_version]):
                pdf_elements.append(Paragraph(f"{code_type} Code Section {i+1}:", styles['Heading3']))
                code_paragraph_style = ParagraphStyle(
                    name='CodeStyle',
                    fontName='Courier',
                    fontSize=8,
